...
float cost_and_grad_single(float* log_probs, float* grads,
                           ...
                           int U, int V, int batch_size) {
    // Forward pass
    float *alphas = (float *) malloc(T * U * sizeof(float));
    alphas[0] = 0;
    for (int t = 1; t < T; t++) {
        alphas[idx2(t, 0, U)] = alphas[idx2(t-1, 0, U)] + log_probs[idx3(t-1, 0, blank, U, V)];
    }

    for (int u = 1; u < U; u++) {
        alphas[idx2(0, u, U)] = alphas[idx2(0, u-1, U)] + log_probs[idx3(0, u-1, labels[u-1], U, V)];
    }

    for (int t = 1; t < T; t++) {
        for (int u = 1; u < U; u++) {
            float no_emit = alphas[idx2(t-1, u, U)] + log_probs[idx3(t-1, u, blank, U, V)];
            float emit = alphas[idx2(t, u-1, U)] + log_probs[idx3(t, u-1, labels[u-1], U, V)];
            alphas[idx2(t, u, U)] = log_sum_exp(emit, no_emit);
        }
    }
    float forward_ll = alphas[idx2(T-1, U-1, U)] + log_probs[idx3(T-1, U-1, blank, U, V)];

    // Backward pass
    float *betas = (float *) malloc(T * U * sizeof(float));
    betas[idx2(T-1, U-1, U)] = log_probs[idx3(T-1, U-1, blank, U, V)];
    for (int t = T-2; t >= 0; t--) {
        betas[idx2(t, U-1, U)] = betas[idx2(t+1, U-1, U)] + log_probs[idx3(t, U-1, blank, U, V)];
    }
    for (int u = U-2; u >= 0; u--) {
        betas[idx2(T-1, u, U)] = betas[idx2(T-1, u+1, U)] + log_probs[idx3(T-1, u, labels[u], U, V)];
    }

    for (int t = T-2; t >= 0; t--) {
        for (int u = U-2; u >= 0; u--) {
            float no_emit = betas[idx2(t+1, u, U)] + log_probs[idx3(t, u, blank, U, V)];
            float emit = betas[idx2(t, u+1, U)] + log_probs[idx3(t, u, labels[u], U, V)];
    ...
    grads[idx3(T-1, U-1, blank, U, V)] = alphas[idx2(T-1, U-1, U)];
    for (int t = 0; t < T-1; t++) {
        for (int u = 0; u < U; u++) {
            grads[idx3(t, u, blank, U, V)] = alphas[idx2(t, u, U)] + betas[idx2(t+1, u, U)];
        }
    }
    for (int t = 0; t < T; t++) {
        for (int u = 0; u < U - 1; u++) {
            int l = labels[u];
            grads[idx3(t, u, l, U, V)] = alphas[idx2(t, u, U)] + betas[idx2(t, u+1, U)];
        }
    }
    for (int t = 0; t < T; t++) {
        for (int u = 0; u < U; u++) {
